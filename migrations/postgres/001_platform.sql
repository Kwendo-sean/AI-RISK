-- PostgreSQL schema. Mirrors migrations/sqlite/001_platform.sql.
-- Epoch seconds are kept as BIGINT so the application code is dialect-neutral.
...
-- Every authenticated read filters on both columns; this index serves them together.
CREATE INDEX IF NOT EXISTS idx_user_profiles_session_token ON user_profiles(session_id, token_hash);
...
CREATE INDEX IF NOT EXISTS idx_assessment_status_created ON assessment_sessions(status, created_at DESC);
